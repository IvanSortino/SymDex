...
<p align="center">
  ...
  MCP client · Cursor · GitHub Copilot · OpenCode · Any agent that speaks MCP.
</p>
...
```bash
symdex serve              # stdio — for MCP desktop client, Cursor, Copilot
...
SymDex speaks the **Model Context Protocol (MCP)** — the open standard for connecting AI agents to external tools, created by MCP project and now adopted across the AI tooling ecosystem. If a platform supports MCP, SymDex works with it.

| Platform | MCP Support | How to Connect |
|----------|-------------|---------------|
| MCP desktop client | Native | Add to `mcp_client_config.json` |
